...
class Tilemap():
    def __init__(self, game, tile_size=16):
        ...
        for i in range(10):
            self.tilemap[str(i + 3) + ':10'] = {'type': 'placeholder', 'pos':(i + 3, 10), 'anim': False}

        for i in range(5):
            self.tilemap[str(i + 8) + ':9'] = {'type': 'test/anim_placeholder', 'pos':(i + 8, 9), 'anim': True}
    ...
    def render(self, surface, scroll_offset=[0,0]):
        for loc in self.tilemap:
            ...
            if tile['anim']:
                surface.blit(self.game.assets[tile['type']].play(), (tile['pos'][0] * self.tile_size - scroll_offset[0],tile['pos'][1] * self.tile_size - scroll_offset[1]))
